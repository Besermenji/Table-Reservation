...
var calendar = function(){
    ...
    var current_resource = function(){
    	return window.location.href.match(/tables\/(\d+)\/bookings/)[1];
    };
    ...
    $('#calendar').fullCalendar({
        header: {
			...
			eventSources: [{  
    		url: '/tables/'+current_resource()+'/bookings/',  
   		}],
    ...
    eventDrop: function(booking) {
      var length = (booking.end-booking.start)/(3600000);
        ...
        function updateEvent(booking) {
              $.ajax(
                '/tables/'+current_resource()+'/bookings/'+booking.id,
                { 'type': 'PATCH',
    ...
    ,
      ...
      if (window.location.href.match(/new/)) {
        if(today_or_later()) {
          ...
          jQuery.post(
            '/tables/'+current_resource()+'/bookings',
